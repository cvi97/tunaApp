...
CREATE TABLE `UserEvent` (
  ...
  CONSTRAINT `fk_event` FOREIGN KEY (`Event`) REFERENCES `Events` (`EventID`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_user` FOREIGN KEY (`User`) REFERENCES `Users` (`UserID`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
